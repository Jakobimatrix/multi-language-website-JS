 ...
 */
LANGUAGES_ARRAY = new Array("GER","ENG","POL", "ESP");         //all suported languages have to be written in this array. NOTE that the classnames have to be equal to these entries
FLAGS_PATH = "https://YOUR_WEBSEITE/js/pictures/";             //the path where all the pictures of the flages are. NOTE that image name has to be GER.jpg like you defined it in the array "LANGUAGES_ARRAY" above
DEFAULT_LANGUAGE = "GER";                                      //set the default language
DONT_SHOW_FLAG_WITH_CURRENT_LANGUAGE = true;                   //if DONT_SHOW_FLAG_WITH_CURRENT_LANGUAGE is true, the flag of the current language wont be displayed
COOKIENAME = "Language";                                       //choose an avaiable name for the cookie

CLASSNAME_FLAGS = "LanguageFlag";                              //choose an avaiable class ame for the flags
...
// set the style (css format) / behavoiur for the unique flags both for desctop and mobile. Dont change the order of the IDs!
var CSS_FLAGS = `
.both{
  cursor: pointer;
  z-index: 100;
...
function setCookie(Language){
    if(Language != DEFAULT_LANGUAGE){
        document.cookie = COOKIENAME+'=' + Language + '; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/';
    }else{
        document.cookie = COOKIENAME+'=' + Language + '; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/';
    }
}
/*
...
function changeLanguage(domObjekt){
    ...
    if(DONT_SHOW_FLAG_WITH_CURRENT_LANGUAGE){ //if we dont want to display the flag with the displayed language, we must erase all flags and set them new
        for(i=0;i<All_DOM_Flags.length;i++){
            document.body.removeChild(All_DOM_Flags[i]);   //remove old Flags
        }
